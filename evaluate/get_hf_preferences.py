...
async def main():
    ...
    args = parse_args()
    judge_llm = os.environ['MODEL']
    judge_name = "Gemma2_27b"
    ...
    print(f"Initializing LLM Judge '{judge_name}' with {ngpus} GPUS.\n")
    llm = LLM(model=judge_llm, tensor_parallel_size=ngpus, distributed_executor_backend='ray') #disable_sliding_window=True)
    ...
    for contest in human_contests:
        ...
        if {"first":a, "second":b, "question":idx, "winner":a} in contest_data or {"first":a, "second":b, "question":idx, "winner":b} in contest_data:
            ...
            try:
                ...
                tokenizer = AutoTokenizer.from_pretrained(judge_llm)
                ...
                #tokenizer = AutoTokenizer.from_pretrained(judge_llm)
                #final = str(tokenizer.apply_chat_template(message, tokenize=False))
